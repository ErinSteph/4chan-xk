...
# 4chan XK
4chan XK is a script that adds various features to anonymous imageboards, forked for circlerjerks. It was originally developed for 4chan but has no affiliation with it.
...
### Chromium
**Userscript**: Install [Violentmonkey](https://chrome.google.com/webstore/detail/violent-monkey/jinjaccalgkegednnccohejagnlnfdag) or [Tampermonkey](https://tampermonkey.net/), then **[click here to install 4chan X](https://github.com/ErinSteph/4chan-xk/blob/master/builds/4chan-X.user.js?raw=true)**.
...
- **Midori**: Enable `User addons` in your preferences, under the Extensions tab. In the Privacy tab, check `Enable HTML5 local storage support`. Optionally, if you want 4chan X to be able to open new tabs when you start or reply to a thread, you will need to check `Allow scripts to open popups` under the Behavior tab. Then click the link to the [script](https://github.com/ErinSteph/4chan-xk/blob/master/builds/4chan-X.user.js?raw=true) to install it.
...
### MS Edge
Install [Tampermonkey](https://www.microsoft.com/en-us/store/p/tampermonkey/9nblggh5162s), then **[click here to install 4chan X](https://github.com/ErinSteph/4chan-xk/blob/master/builds/4chan-X.user.js?raw=true)**.
